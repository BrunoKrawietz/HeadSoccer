...
function render(currentTime) {
    ...
    // Keep the grass slightly above the support platform to avoid z-fighting/flicker.
...
function createTransform(translation, scale = [1, 1, 1], rotation = [0, 0, 0]) {
...
function renderArena() {
    ...
    renderSolidObject(createTransform([0, 1.42, 0.09], [9.25, 0.12, 0.18]), buffers.cube, wallColor, 0.25);
    renderSolidObject(createTransform([0, -1.42, 0.09], [9.25, 0.12, 0.18]), buffers.cube, wallColor, 0.25);
    // Lower the dark platform so it no longer overlaps with the textured grass surface.
    renderSolidObject(createTransform([0, 0, -0.08], [9.25, 2.75, 0.04]), buffers.cube, [0.05, 0.20, 0.08], 0.08);
...
function renderGoals() {
    ...
    renderSolidObject(createTransform([a.right + 0.34, 0, rightGoalZ/2], [post, goalY*2, rightGoalZ]), buffers.goalPost, netColor, 0.06);
}

function renderPlayer(player, shirtColor, darkColor) {
    const root = mat4.create();
    mat4.translate(root, root, [player.position.x, 0, player.position.z + 0,7]);
...
function renderLimb(root, hip, scale, angle, color) {
    ...
    renderSolidObject(limb, buffers.cube, color, 0.22);
}
...
function renderLeg(root, hip, angle, pantsColor, shoeColor, direction) {
    ...
    // Shorter lower body geometry so the player visibly stands on the pitch.
...
function renderSolidObject(modelMatrix, bufferSet, color, specularStrength = 0.35) {
    gl.useProgram(solidProgram);
